...
function subTopologyId(topologyId::UInt, dim::Int, codim::Int, i::UInt)::UInt
  @argcheck i < _size(topologyId, dim, codim)
  mydim = dim - codim
...
function subTopologyNumbering(topologyId::UInt, dim::Int, codim::Int, i::UInt, subcodim::Int, out::AbstractVector{UInt})
  ...
  @argcheck i <= _size(topologyId, dim, codim)
  @argcheck length(out) == _size(subTopologyId(topologyId, dim, codim, i), dim-codim, subcodim)
  ...
  elseif subcodim == 0
    ...
    baseId = baseTopologyId(topologyId, dim)
    m = _size(baseId, dim-1, codim-1)
    mb = _size(baseId, dim-1, codim+subcodim-1)
    nb = (codim + subcodim < dim ? _size(baseId, dim-1, codim+subcodim) : 0)

    if isPrism(topologyId, dim)
      n = _size(baseId, dim-1, codim)
      if i < n
        subId = subTopologyId(baseId, dim-1, codim, i)

        beginBase = beginOut
        if codim + subcodim < dim
          beginBase = beginOut + _size(subId, dim-codim-1, subcodim)
          subTopologyNumbering(baseId, dim-1, codim, i, subcodim, view(out,beginOut:beginBase))
        end

        ms = _size(subId, dim-codim-1, subcodim-1)
...
function _checkInside{T<:Real}(topologyId::UInt, dim::Int, x::AbstractVector{T}, tolerance::T, factor::T = 1::T)::Bool
  @argcheck 0 <= dim <= cdim
  ...
  if dim > 0
    ...
    if (x[dim] > -tolerance) && (factor - x[dim] > -tolerance)
      return _checkInside(baseTopologyId(topologyId, dim), dim-1, x, tolerance, baseFactor)
    else
      return false
    end
  else
...
function referenceCorners{T<:Real}(topologyId::UInt, dim::Int, corners::AbstractVector{AbstractArray{T,1}})::UInt
  ...
  if dim > 0
    ...
    @assert nBaseCorners == _size(baseTopologyId(topologyId, dim), dim-1, dim-1)
...
function referenceIntegrationOuterNormals{T<:Real}(topologyId::UInt, dim::Int,
  ...
  if dim > 1
    ...
    for i = 1:2
      normals[i] = zeros(T,Base.size(normals[i]))
...
function referenceIntegrationOuterNormals{T<:Real}(topologyId::UInt, dim::Int,
  ...
  origins = [Vector{T}(undef, cdim) for _ in 1:_size(topologyId, dim, 1)]
  referenceOrigins(topologyId, dim, 1, origins)

  numFaces = referenceIntegrationOuterNormals(topologyId, dim, origins, normals)
  @assert numFaces == _size(topologyId, dim, 1)

  return numFaces
end


function maxSubEntityCount(dim::Int)::Int
  maxCount = 0::Int
  for codim = 0:dim
    maxCount = Base.max(maxCount, Base.binomial(dim,codim) * (1 << codim))
  end
  return maxCount
end
...
struct SubEntityInfo
  ...
  "Constructor"
  SubEntityInfo(dim::Int) = new(dim, undef,   # dimension, numbering_
...
function size(self::SubEntityInfo, cc::Int)::UInt
  @argcheck 0 <= cc <= dim
  return self.offset_[cc+2] - self.offset_[cc+1]
end


function number(self::SubEntityInfo, ii::Int, cc::Int)::UInt
  @argcheck 0 < ii <= size(self, cc)
  return self.numbering_[self.offset_[cc+1] + ii]
end


function numbers(self::SubEntityInfo, cc::Int)::UInt
  return (Base.view(self.numbering_, self.offset_[cc+1]:self.offset_[cc+2]), self.containsSubentity_[cc+1])
end


function initialize(self::SubEntityInfo, topologyId::UInt, codim::Int, i::Int)
  dim = self.dimension
  subId = subTopologyId(topologyId, dim, codim, i)
  self.type_ = GeometryType(subId, dim-codim)

  # compute offsets
  self.offset_ = Base.zeros(UInt, Base.size(self.offset_))
  for cc = codim:dim
    self.offset_[cc+2] = self.offset_[cc+1] + _size(subId, dim-codim, cc-codim)
  end

  # compute subnumbering
  self.numbering_ = Vector(undef, self.offset_[dim+2])
  for cc = codim:dim
  ...
  for cc = 0:dim
    ...
    for idx = 1:size(self,cc)
      containsSubentity_[cc+1][number(self,idx,cc)] = true
    end
  end
end